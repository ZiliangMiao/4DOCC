...
def MosCollateFn(batch):
    # batch is a list which contains tuple returned by dataloader getitem method
    filenames = [item[0] for item in batch]
    ...
    # mos labels
    mos_labels = torch.stack(
        [
            torch.nn.functional.pad(
                item[3],
                (0, max_n_input_points - len(item[3])),
                mode="constant",
                value=255,  # uint8, -1 = 255
            )
            for item in batch
        ]
    )
    return (filenames, input_points, input_tindex, mos_labels)
