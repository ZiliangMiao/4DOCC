...
def draw(query_rays, key_rays_list, query_ray_to_ints_pts_tensor_dict, key_sensor_idx_to_ray_idx_dict):
    ...
    for key_idx, ints_rays_idx in key_sensor_idx_to_ray_idx_dict.items():
        ...
        # axis
        org_key = key_rays.get_ray_start()
...
def draw_dynamic_rays(query_rays, key_rays_list, rays_to_ints_pts_dict, mos_labels_query, mos_labels_key_list):
    ...
    def draw_ray(vis):
        ...
        for key_rays in key_rays_list:
            key_org = key_rays.get_ray_start()
            ...
            key_rays_points_list.append(key_rays.get_ray_end())
        ...
        query_ray_point = np.asarray(query_rays.get_ray_end()[query_ray_idx].numpy())
        ...
        for i in range(len(key_sensor_rays_idx)):
            ...
            key_ray_point = key_rays_points_list[key_sensor_idx][key_ray_idx].numpy()
        ...
        # line_mesh = LineMesh(vertex_query, lines_query, [0, 0, 0], radius=0.2)
...
def get_transformed_pcd(nusc, sd_token_ref, sd_token, max_range):
    ...
    intensity = torch.tensor(lidar_pcd.points[-1, :].T, dtype=torch.float32)
    ...
    mos_labels = np.fromfile(mos_labels_file, dtype=np.uint8)[filter_mask.numpy()]
    return origin_tf, points_tf, ts_rela, intensity, mos_labels
...
class QueryRays(object):
    ...
    def cal_ints_points(self, key_rays_list:list, occ_thrd:float):
        ...
        for key_sensor_idx, key_rays in enumerate(key_rays_list):  # key_rays at different space and time
            ...
            query_rays_dir = self.ray_dir[query_ints_idx]  # query rays (have intersection points)
            ...
            # query_ints_idx = query_ints_idx[valid_ints_mask].numpy()
            # key_ints_idx = key_ints_idx[valid_ints_mask].numpy()
            # q = q[valid_ints_mask]
            # k = k[valid_ints_mask]
            ...
            q = q[valid_ints_mask]
            k = k[valid_ints_mask]
            ints_pts = self.ray_start + q.reshape(-1, 1) * self.ray_dir[query_ints_idx]
            ...
            ints_label = torch.zeros(len(ints_pts))  # 0: unknown, 1: free, 2:occupied
            ...
            ints_pts[ints_adv_idx] = self.ray_start + q.reshape(-1, 1)[ints_adv_idx] * \
                                     self.ray_dir[query_ints_idx][ints_adv_idx]  # update ints pts that ints in advance
            ...
            #      key_ints_idx.reshape(-1, 1), ints_pts.numpy(),
            #      np.broadcast_to(key_rays.get_ray_ts(), (num_ints_all, 1)),
            #      ints_label.numpy().reshape(-1, 1)), axis=1)
            # # TODO: save valid labels (only free and occupied)
            # valid_label_idx = torch.cat(occ_idx + free_idx + ints_adv_idx).numpy()
            ...
            #      key_ints_idx[valid_label_idx].reshape(-1, 1), ints_pts.numpy()[valid_label_idx],
            #      np.broadcast_to(key_rays.get_ray_ts(), (num_ints_valid, 1)),
            #      ints_label.numpy()[valid_label_idx].reshape(-1, 1)), axis=1)

            valid_label_idx = torch.cat(occ_idx + free_idx + ints_adv_idx).numpy()
            ints_pts_ts_label = torch.cat((ints_pts[valid_label_idx],
                                           torch.full((len(valid_label_idx), 1), key_rays.get_ray_ts()),
                                           ints_label[valid_label_idx].reshape(-1, 1)), dim=1)
...
if __name__ == '__main__':
    ...
    for sample_idx, sample_query in tqdm(enumerate(nusc.sample)):
        ...
        # get original data
        org_query, pts_query, ts_query, intensity_query, mos_labels_query = get_transformed_pcd(nusc, sd_tok_query, sd_tok_query, max_range=max_range)
        query_rays = QueryRays(org_query, pts_query, ts_query)
        key_rays_list = []
        mos_labels_key_list = []
        for sd_tok_key in sd_tok_key_list:
            org_key, pts_key, ts_key, intensity_key, mos_labels_key = get_transformed_pcd(nusc, sd_tok_query, sd_tok_key, max_range=max_range)
        ...
        if open3d_vis:
            ...
            query_ray_samples, ints_pts_samples = query_rays.cal_ints_points(key_rays_list, occ_thrd)
