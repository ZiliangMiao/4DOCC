...
class NuscSequentialDataset(Dataset):
    def __init__(self, cfg, nusc, split):
        ...
        self.data_pct = self.cfg["data"]["dataset_pct"]

        split_scenes = create_splits_scenes(verbose=True)
        split_scenes = split_scenes[self.split]

        ####################################################
        # randomly select a certain percentage of sequences
        from random import sample
        split_scenes = sample(split_scenes, int(len(split_scenes) * self.data_pct / 100))
        ####################################################
