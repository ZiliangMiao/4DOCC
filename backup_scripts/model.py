...
class OccupancyForecastingNetwork(nn.Module):
    ...
    def forward(
        ...
        input_points = ((input_points_orig - self.offset) / self.scaler).float()  # to float voxel index
        ...
        # N x T1 x H x L x W
        # init: init_cuda_kernel method in dvr.cu
        input_occupancy = dvr.init(input_points, input_tindex, self.input_grid)
        input_occ = input_occupancy.cpu().numpy()
        in_occ_b1_t1 = input_occ[0][0]
        in_occ_max = np.max(in_occ_b1_t1)
        in_occ_min = np.min(in_occ_b1_t1)
        ...
        # N, (Z * T), Y, X
        _input = input_occupancy.reshape(N, -1, L, W)  # standard occ grid: 2 * 45 * 700 * 700
